...
from app.domain.models.account import (  # Giữ lại cho mục đích type hinting nếu cần
    LoaiTaiKhoan,
    TaiKhoan,
)
...
class JournalingService:
    ...
    def _kiem_tra_khoa_so(self, ngay_ct: date):
        ...
        """
        ky_ke_toan = self.accounting_period_service.lay_ky_ke_toan_theo_ngay(
            ngay_ct
        )
        if not ky_ke_toan:
            ...
            print(
                f"[WARN] Ngày {ngay_ct} không thuộc bất kỳ kỳ kế toán nào. Cho phép tạo."
            )
            return

        if ky_ke_toan.trang_thai == "Locked":
            raise ValueError(
                f"Kỳ kế toán '{ky_ke_toan.ten_ky}' (Ngày {ngay_ct.strftime('%Y-%m-%d')}) đã bị khóa. Không thể thực hiện nghiệp vụ."
            )
    ...
    def _kiem_tra_tai_khoan_ton_tai(self, lines: List[JournalEntryLine]):
        ...
        for line in lines:
            ...
            if not tai_khoan:
                raise ValueError(
                    f"Tài khoản '{line.so_tai_khoan}' không tồn tại trong hệ thống."
                )
    ...
    def cap_nhat_phieu_ke_toan(
        ...
        if journal_entry_hien_tai.trang_thai != "Draft":
            raise ValueError(
                f"Không thể cập nhật bút toán ID {id} vì trạng thái là '{journal_entry_hien_tai.trang_thai}'. Chỉ có thể cập nhật bút toán ở trạng thái 'Draft'."
            )
        ...
        if (
            journal_entry_domain_updated.ngay_ct
            != journal_entry_hien_tai.ngay_ct
        ):
    ...
    def xoa_phieu_ke_toan(self, id: int) -> bool:
        ...
        if journal_entry.trang_thai != "Draft":
            raise ValueError(
                f"Không thể xóa bút toán ID {id} vì trạng thái là '{journal_entry.trang_thai}'. Chỉ có thể xóa bút toán ở trạng thái 'Draft'."
            )
    ...
    def unpost_phieu_ke_toan(self, id: int) -> JournalEntryDomain:
        ...
        if journal_entry.trang_thai == "Draft":
            raise ValueError(
                f"Bút toán ID {id} đang ở trạng thái Draft, không cần hủy đăng sổ."
            )
        if journal_entry.trang_thai == "Locked":
            raise ValueError(
                f"Bút toán ID {id} đã bị khóa, không thể thay đổi trạng thái."
    ...
    def _tinh_phat_sinh_tai_khoan(
        ...
        all_entries = self.repository.get_all_posted_in_range(
            ngay_dau_nam, ngay_ket_nam
        )
        tong = Decimal(0)
