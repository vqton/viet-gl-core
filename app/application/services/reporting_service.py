...
from app.application.services.accounting_period_service import (  # Cần cho việc xác định kỳ
...
class ReportingService:
    ...
    def __init__(
        ...
        self.period_service = (
            period_service  # Dùng để xác định kỳ kế toán (nếu cần)
        )

    def _get_opening_balance(
        self, so_tai_khoan: str, ngay_bat_dau: date
    ) -> Decimal:
        """
    ...
    def _tinh_so_du_tai_khoan_theo_ngay(
        ...
        journal_entries = self.journal_entry_repo.get_all_posted_in_range(
        ...
        if loai_tai_khoan in [
            LoaiTaiKhoan.TAI_SAN,
            LoaiTaiKhoan.CHI_PHI,
            LoaiTaiKhoan.GIA_VON,
        ]:
    ...
    # Trial Balance
    # ----------------------------
    def lay_bang_can_doi_so_phat_sinh(
        self,
        ky_hieu: str,
        ngay_lap: date,
        ngay_bat_dau: date,
        ngay_ket_thuc: date,
    ) -> List[ChiTietTaiKhoan]:
        all_accounts = self.account_repo.get_all()
        result_details: List[ChiTietTaiKhoan] = []

        for tai_khoan in all_accounts:
            sd_dau_ky, ps_no, ps_co, sd_cuoi_ky_no, sd_cuoi_ky_co = (
                self._tinh_so_du_tai_khoan_theo_ngay(
            ...
            sd_dk_no = Decimal(0)
            sd_dk_co = Decimal(0)
            if tai_khoan.loai_tai_khoan in [
                LoaiTaiKhoan.TAI_SAN,
                LoaiTaiKhoan.CHI_PHI,
                LoaiTaiKhoan.GIA_VON,
            ]:
    ...
    def lay_bao_cao_tinh_hinh_tai_chinh(
        ...
        tien_va.tong_cong = (
            tien_va.tien_mat
            + tien_va.tien_gui_ngan_hang
            + tien_va.tien_dang_chuyen
        ).quantize(QUANT)

        # Tai san ngan han
        cac_khoan_phai_thu_ngan_han = (
            get_balance('131') + get_balance('138')
        ).quantize(QUANT)
        hang_ton_kho = (
            get_balance('152')
            + get_balance('153')
        ...
        tai_san_co_dinh_huu_hinh = (
            get_balance('211') - get_balance('214')
        ).quantize(QUANT)
        tai_san_co_dinh_vo_hinh = get_balance('221')
        bat_dong_san_dau_tu = get_balance('217')
        cac_khoan_dau_tu_tai_chinh_dai_han = get_balance(
            '221'
        )  # nếu khác map lại
        tai_san_dai_han_khac = (
            get_balance('241') + get_balance('242')
        ).quantize(QUANT)
        ...
        tong_tai_san_total = (
            tong_tai_san_ngan_han + tong_tai_san_dai_han
        ).quantize(QUANT)
        ...
        no_ngan_han_obj = NoPhaiTraNganHan(
            ...
            phai_tra_ngan_han_khac=(
                get_balance('334') + get_balance('338')
            ).quantize(QUANT),
        ...
        tong_no_phai_tra = (phai_tra_ngan_han + get_balance('341')).quantize(
            QUANT
        )
        ...
        von_obj = VonChuSoHuu(
            ...
            tong_von_chu_so_huu=(
                get_balance('411') + get_balance('421')
            ).quantize(QUANT),
        )

        tong_nguon_von_total = (
            tong_no_phai_tra + von_obj.tong_von_chu_so_huu
        ).quantize(QUANT)
        ...
        ).quantize(QUANT, rounding=ROUND_HALF_UP) != Decimal(0):
            print(
                f"[CẢNH BÁO] Bảng Cân Đối không cân bằng! TS: {tong_tai_san.tong_cong_tai_san}, NV: {tong_nguon_von.tong_cong_nguon_von}"
            )
    ...
    def lay_bao_cao_ket_qua_hdkd(
        ...
        doanh_thu_thuan = (doanh_thu_ban_hang - giam_tru_doanh_thu).quantize(
            QUANT
        )
        ...
        tong_loi_nhuan_truoc_thue = (
            loi_nhuan_thuan_tu_hdkd + loi_nhuan_khac
        ).quantize(QUANT)
        chi_phi_thue_thu_nhap_doanh_nghiep_hien_hanh = get_ps('821', 'NO')
        loi_nhuan_sau_thue = (
            tong_loi_nhuan_truoc_thue
            - chi_phi_thue_thu_nhap_doanh_nghiep_hien_hanh
        ).quantize(QUANT)
    ...
    def lay_bao_cao_luu_chuyen_tien_te(
        ...
        b02 = self.lay_bao_cao_ket_qua_hdkd(
        ...
        tien_chi_mua_tscd = self._tinh_chenh_lech_tai_san_dai_han(
            "211", ngay_bat_dau, ngay_ket_thuc
        )

        # b) Tiền thu thanh lý, nhượng bán TSCĐ (mã 22)
        tien_thu_ban_tscd = Decimal(
            0
        )  # Cần tích hợp từ bút toán thanh lý (tạm hardcode)

        # c) Tiền chi cho vay, mua công cụ nợ (mã 24)
        tien_chi_cho_vay = self._tinh_phat_sinh_tai_khoan(
            "121", "NO", ngay_bat_dau, ngay_ket_thuc
        )  # Đầu tư ngắn hạn

        # d) Tiền thu hồi cho vay, bán lại công cụ nợ (mã 25)
        tien_thu_hoi_vay = self._tinh_phat_sinh_tai_khoan(
            "121", "CO", ngay_bat_dau, ngay_ket_thuc
        )
        ...
        tien_thu_vay = self._tinh_chenh_lech_no_dai_han(
            "341", ngay_bat_dau, ngay_ket_thuc
        )

        # c) Tiền chi trả gốc vay (mã 33)
        tien_chi_tra_goc_vay = self._tinh_tra_goc_vay(
            "341", ngay_bat_dau, ngay_ket_thuc
        )

        # d) Tiền chi trả cổ tức, lợi nhuận (mã 36)
        tien_chi_tra_co_tuc = self._tinh_phat_sinh_tai_khoan(
            "3387", "CO", ngay_bat_dau, ngay_ket_thuc
        )  # Doanh thu chưa thực hiện chi trả
        ...
        tien_cuoi_ky = (tien_dau_ky + luu_chuyen_thuan_trong_ky).quantize(
            Decimal("0.01")
        )
    ...
    def lay_bao_cao_thuyet_minh(
        ...
        bang_can_doi = self.lay_bang_can_doi_so_phat_sinh(
            ky_hieu, ngay_lap, ngay_bat_dau, ngay_ket_thuc
        )

        chi_tiet_131 = [
            d for d in bang_can_doi if d.so_tai_khoan.startswith('131')
        ]
        thuyet_minh_tai_san = ThuyetMinhTaiSan(
            tong_cong_thuyet_minh=sum(
                [d.so_du_cuoi_ky_no + d.so_du_cuoi_ky_co for d in chi_tiet_131]
            ),
        ...
        chi_tiet_331 = [
            d for d in bang_can_doi if d.so_tai_khoan.startswith('331')
        ]
        thuyet_minh_nguon_von = ThuyetMinhNguonVon(
            tong_cong_thuyet_minh=sum(
                [d.so_du_cuoi_ky_no + d.so_du_cuoi_ky_co for d in chi_tiet_331]
            ),
        ...
        chi_tiet_doanh_thu = [
            d for d in bang_can_doi if d.so_tai_khoan.startswith('511')
        ]
    ...
    def _tinh_thay_doi_so_du(
        self, so_tai_khoan: str, ngay_bat_dau: date, ngay_ket_thuc: date
    ) -> Decimal:
        """Tính thay đổi số dư = Số dư cuối kỳ - Số dư đầu kỳ."""
        # Số dư đầu kỳ
        _, _, _, sd_dk_no, sd_dk_co = self._tinh_so_du_tai_khoan_theo_ngay(
            so_tai_khoan, ngay_bat_dau, ngay_bat_dau
        )
        sd_dk = (
            sd_dk_no - sd_dk_co
            if sd_dk_no >= sd_dk_co
            else -(sd_dk_co - sd_dk_no)
        )

        # Số dư cuối kỳ
        _, _, _, sd_ck_no, sd_ck_co = self._tinh_so_du_tai_khoan_theo_ngay(
            so_tai_khoan, ngay_bat_dau, ngay_ket_thuc
        )
        sd_ck = (
            sd_ck_no - sd_ck_co
            if sd_ck_no >= sd_ck_co
            else -(sd_ck_co - sd_ck_no)
        )

        return (sd_ck - sd_dk).quantize(Decimal("0.01"))

    def _tinh_chenh_lech_tai_san_dai_han(
        self, so_tai_khoan: str, ngay_bat_dau: date, ngay_ket_thuc: date
    ) -> Decimal:
        ...
        return (
            chenh_lech if chenh_lech > 0 else Decimal(0)
        )  # Chỉ lấy phần tăng

    def _tinh_chenh_lech_no_dai_han(
        self, so_tai_khoan: str, ngay_bat_dau: date, ngay_ket_thuc: date
    ) -> Decimal:
    ...
    def _tinh_tra_goc_vay(
        self, so_tai_khoan: str, ngay_bat_dau: date, ngay_ket_thuc: date
    ) -> Decimal:
        """Tính tiền chi trả gốc vay = Phát sinh Nợ TK 341"""
        return self._tinh_phat_sinh_tai_khoan(
            so_tai_khoan, "NO", ngay_bat_dau, ngay_ket_thuc
        )
