...
def get_journaling_service(
    db: Session = Depends(get_db),
    accounting_period_service: AccountingPeriodService = Depends(
        get_accounting_period_service
    ),
) -> JournalingService:
    journal_repo = JournalEntryRepository(db)
    ...
    return JournalingService(
        repository=journal_repo,
        account_repository=account_repo,
        accounting_period_service=accounting_period_service,
    )
...
def get_reporting_service(db: Session = Depends(get_db)) -> ReportingService:
    ...
    return ReportingService(
        journal_entry_repo=journal_repo, account_repo=account_repo
    )
...
@router.get(
    "/accounts", response_model=List[TaiKhoanDomain], tags=["Accounting - COA"]
)
...
def lay_ky_ke_toan(
    ...
    if not ky:
        raise HTTPException(
            status_code=404, detail="Không tìm thấy kỳ kế toán"
        )
    return ky
...
@router.get(
    "/accounting-periods",
    response_model=List[KyKeToanDomain],
    tags=["Accounting - Period Control"],
)
...
@router.post(
    "/accounting-periods/{id}/lock", tags=["Accounting - Period Control"]
)
def khoa_ky(
    id: int,
    nguoi_thuc_hien: str = Body(
        default="System", embed=True
    ),  # Nhận người thực hiện từ body (tùy chọn)
    service: AccountingPeriodService = Depends(get_accounting_period_service),
):
    """
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=400,
                detail="Không thể khóa kỳ (có thể đã bị khóa hoặc có bút toán chưa ghi sổ).",
            )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi khóa kỳ: {e}"
        )


@router.post(
    "/accounting-periods/{id}/unlock", tags=["Accounting - Period Control"]
)
def mo_ky(
    id: int,
    ...
    """
    try:
        success = service.mo_ky(
            id, ly_do=reason, nguoi_thuc_hien=nguoi_thuc_hien
        )
        if success:
            return {
                "message": f"Kỳ {id} đã được mở thành công. Lý do: {reason}"
            }
        else:
            raise HTTPException(
                status_code=400,
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi mở kỳ: {e}"
        )
...
@router.post(
    "/journal-entries",
    response_model=JournalEntryDomain,
    status_code=status.HTTP_201_CREATED,
...
@router.get(
    "/journal-entries",
    response_model=List[JournalEntryDomain],
    tags=["Accounting - Journal Entries"],
)
...
def cap_nhat_phieu_ke_toan(
    ...
    try:
        ...
        if not updated_entry:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy bút toán"
            )
...
def xoa_phieu_ke_toan(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy bút toán để xóa"
            )
        return None
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def ghi_so_phieu_ke_toan(
    ...
    try:
        ...
        if not posted_entry:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy bút toán để ghi sổ"
            )
        return posted_entry
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi ghi sổ: {e}"
        )
...
def huy_ghi_so_phieu_ke_toan(
    ...
    try:
        ...
        if not unposted_entry:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy bút toán để hủy ghi sổ"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi hủy ghi sổ: {e}"
        )
...
@router.get(
    ...
    response_model=BaoCaoTinhHinhTaiChinh,
...
def lay_bao_cao_tinh_hinh_tai_chinh(
    ...
    """
    try:
        return service.lay_bao_cao_tinh_hinh_tai_chinh(
            ky_hieu=ky_hieu, ngay_lap=ngay_lap
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Lỗi hệ thống khi tạo báo cáo: {e}"
        )


@router.get(
    "/reports/performance",
    response_model=BaoCaoKetQuaHDKD,
    tags=["Accounting - Reports"],
)
def lay_bao_cao_ket_qua_hdkd(
    ...
    """
    try:
        return service.lay_bao_cao_ket_qua_hdkd(
            ky_hieu=ky_hieu, ngay_lap=ngay_lap
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get(
    "/reports/cash-flow",
    response_model=BaoCaoLuuChuyenTienTe,
    tags=["Accounting - Reports"],
)
def lay_bao_cao_luu_chuyen_tien_te(
    ...
    """
    try:
        return service.lay_bao_cao_luu_chuyen_tien_te(
            ky_hieu=ky_hieu, ngay_lap=ngay_lap
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@router.get(
    "/reports/disclosure",
    response_model=BaoCaoThuyetMinh,
    tags=["Accounting - Reports"],
)
