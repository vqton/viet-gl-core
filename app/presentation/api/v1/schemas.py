...
class TaiKhoanCreate(BaseSchema):
    ...
    so_tai_khoan: str = Field(..., max_length=20, description="Số tài khoản (Ví dụ: 111, 641)")
    ten_tai_khoan: str = Field(..., max_length=256, description="Tên tài khoản (Ví dụ: Tiền mặt)")
    loai_tai_khoan: LoaiTaiKhoan
    cap_tai_khoan: int = Field(default=1, ge=1, le=9, description="Cấp tài khoản (1, 2, 3...)")
    so_tai_khoan_cha: Optional[str] = Field(None, max_length=20, description="Số tài khoản cha (nếu có)")
    la_tai_khoan_tong_hop: bool = Field(default=True, description="Là tài khoản tổng hợp (true) hay chi tiết (false)")
...
class JournalEntryLineSchema(BaseSchema):
    ...
    so_tai_khoan: str = Field(..., max_length=20, description="Số tài khoản ghi Nợ hoặc Có")
    ...
    mo_ta: Optional[str] = Field(None, max_length=256, description="Mô tả chi tiết dòng (tùy chọn)")

class JournalEntryCreate(BaseSchema):
    """Schema cho việc tạo mới Bút toán."""
    ngay_ct: date = Field(..., description="Ngày chứng từ")
    so_phieu: str = Field(..., max_length=50, description="Số hiệu chứng từ (phải là duy nhất)")
    mo_ta: Optional[str] = Field(None, max_length=512, description="Mô tả chung cho bút toán")
    lines: List[JournalEntryLineSchema] = Field(..., min_length=2, description="Danh sách các dòng bút toán (ít nhất 2 dòng)")
