...
def main():
    ...
    try:
        ...
        print(
            f"{'Mã TK':<10} | {'Tên Tài Khoản':<35} | {'Phát sinh Nợ':>15} | {'Phát sinh Có':>15}"
        )
        print("-" * 85)
        ...
        for row in tb.get("rows", []):
            print(
                f"{row['id']:<10} | {row['name']:<35} | {row['debit']:>15,.0f} | {row['credit']:>15,.0f}"
            )

        print("-" * 85)
        print(
            f"{'TỔNG CỘNG:':<48} | {tb.get('grand_total_debit', 0):>15,.0f} | {tb.get('grand_total_credit', 0):>15,.0f}"
        )
        print("=" * 85)
    except Exception as e:
