...
import pytest  # 👈 Thêm import pytest

from app.domain.models.account import LoaiTaiKhoan, TaiKhoan


def test_khoi_tao_thanh_cong_cap_1():
    """
    ...
    assert tai_khoan.so_tai_khoan == "111"
    assert tai_khoan.ten_tai_khoan == "Tiền mặt"
    assert tai_khoan.loai_tai_khoan == LoaiTaiKhoan.TAI_SAN
    assert tai_khoan.cap_tai_khoan == 1
    assert tai_khoan.la_tai_khoan_tong_hop is True
    assert tai_khoan.so_tai_khoan_cha is None


def test_khoi_tao_thanh_cong_cap_2_co_cha():
    """
    ...
    assert tai_khoan.so_tai_khoan == "1331"
    assert tai_khoan.so_tai_khoan_cha == "133"


def test_so_tai_khoan_trong():
    """
    Test khởi tạo thất bại khi so_tai_khoan trống.
    """
    with pytest.raises(ValueError, match="Số tài khoản không được để trống"):
        TaiKhoan(
            so_tai_khoan="",  # Trống
...
def test_cap_tai_khoan_nho_hon_1():
    """
    Test khởi tạo thất bại khi cap_tai_khoan < 1.
    """
    with pytest.raises(ValueError, match="Cấp tài khoản phải từ 1 đến 3"):
        TaiKhoan(
            so_tai_khoan="123",
            ten_tai_khoan="Một tài khoản",
            loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA,
...
def test_cap_tai_khoan_lon_hon_3():
    """
...
# ✅ Hàm test chuẩn TT99 đã được chuyển sang pytest function
def test_tai_khoan_chuan_tt99():
    ...
    tk_341 = TaiKhoan(
        so_tai_khoan="341",
        ten_tai_khoan="Vay và nợ thuê TC",
        loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA,
        cap_tai_khoan=1,
    )
    ...
    print(
        "✅ test_tai_khoan_chuan_tt99: Tất cả tài khoản đều tuân thủ Phụ lục II TT99."
    )
