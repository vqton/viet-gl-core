...
def sample_accounts():
    ...
    return [
        # --- TÀI SẢN ---
        TaiKhoan(so_tai_khoan="111", ten_tai_khoan="Tiền mặt", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN),
        TaiKhoan(so_tai_khoan="112", ten_tai_khoan="Tiền gửi NH", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN),
        TaiKhoan(so_tai_khoan="156", ten_tai_khoan="Hàng hóa", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN),
        
        # --- NỢ PHẢI TRẢ ---
        TaiKhoan(so_tai_khoan="331", ten_tai_khoan="Phải trả NCC", loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA),
        TaiKhoan(so_tai_khoan="333", ten_tai_khoan="Thuế và các khoản phải nộp NN", loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA),  
        TaiKhoan(so_tai_khoan="3331", ten_tai_khoan="Thuế GTGT phải nộp", loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA),
        
        # --- VỐN CHỦ SỞ HỮU ---
        TaiKhoan(so_tai_khoan="411", ten_tai_khoan="Vốn đầu tư CSH", loai_tai_khoan=LoaiTaiKhoan.VON_CHU_SO_HUU),
        TaiKhoan(so_tai_khoan="421", ten_tai_khoan="Lợi nhuận sau thuế chưa phân phối", loai_tai_khoan=LoaiTaiKhoan.VON_CHU_SO_HUU),
        
        # --- DOANH THU / CHI PHÍ (cho BCTC KQHDKD) ---
        TaiKhoan(so_tai_khoan="511", ten_tai_khoan="Doanh thu bán hàng", loai_tai_khoan=LoaiTaiKhoan.DOANH_THU),
        TaiKhoan(so_tai_khoan="632", ten_tai_khoan="Giá vốn hàng bán", loai_tai_khoan=LoaiTaiKhoan.GIA_VON),
        TaiKhoan(so_tai_khoan="821", ten_tai_khoan="Chi phí thuế TNDN", loai_tai_khoan=LoaiTaiKhoan.CHI_PHI),
    ]
...
def test_lay_bao_cao_tinh_hinh_tai_chinh(mock_repos, sample_accounts, sample_journal_entries):
    ...
    def mock_tinh_so_du(so_tai_khoan, ngay_bat_dau, ngay_ket_thuc):
        balance_map = {
            ...
            "3331": (Decimal("0"), Decimal("10000000")), # Thuế GTGT → THIẾU DÒNG NÀY TRƯỚC ĐÂY!
    ...
    assert abs(balance_diff) < Decimal("0.01"), f"Cân đối kế toán không đúng: TS={bao_cao.tai_san.tong_cong_tai_san}, NV={bao_cao.nguon_von.tong_cong_nguon_von}"
...
def test_lay_bao_cao_ket_qua_hdkd(mock_repos, sample_accounts, sample_journal_entries):
    ...
    assert bao_cao.gia_von_hang_ban == Decimal("0")  # chưa có dữ liệu giá vốn trong sample
...
def test_lay_bang_can_doi_so_phat_sinh(mock_repos, sample_accounts, sample_journal_entries):
    ...
    assert account_112.so_du_dau_ky_co == Decimal("0")  # tài sản -> số dư đầu kỳ nợ
