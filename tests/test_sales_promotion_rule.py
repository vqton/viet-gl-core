...
Yêu cầu pháp lý:
- Hướng dẫn TK 511 TT 99: Hàng khuyến mãi phải được ghi nhận doanh thu theo giá trị thị trường
- Điều 19 TT 99: Doanh thu = Tổng giá trị hàng hóa thực tế (kể cả khi không thu tiền)
- Điều 27 TT 99: Phải tạo công nợ chi tiết cho toàn bộ giá trị hóa đơn

Mục tiêu test:
- Xác minh hệ thống tính doanh thu bao gồm cả hàng khuyến mãi
- Kiểm tra xử lý thuế GTGT cho hàng khuyến mãi
- Đảm bảo công nợ được tạo đúng tổng giá trị
"""
...
def test_sales_with_promotional_items_includes_revenue():
    ...
    # 2. Cấu hình mock tồn kho
    inventory_repo.get_transactions_by_sku.return_value = [
        InventoryTransaction("PHONE01", Decimal('5'), Decimal('7000000'), date(2026, 4, 1), "IN"),
        InventoryTransaction("CHARGER01", Decimal('10'), Decimal('50000'), date(2026, 4, 1), "IN")
    ]
    
    # 3. Cấu hình mock chính sách kế toán
    policy_service.get_current_policy.return_value = AccountingPolicyDTO(
        inventory_valuation_method=ValuationMethod.FIFO,
        effective_date="2026-01-01"
    )
    
    # 4. Tạo hóa đơn có hàng khuyến mãi
    ...
    dto = SalesInvoiceDTO(
        ...
        line_items=[
            ...
            SalesLineItem("CHARGER01", "Sạc (KM)", Decimal('1'), Decimal('100000'))  # Hàng KM
        ]
    )
...
def test_free_promotional_item_still_generates_revenue():
    ...
    - Đây là kiểm tra phòng vệ cho dữ liệu đầu vào sai
    """
    # 1. Chuẩn bị dependencies
    inventory_repo = Mock()
    journal_repo = Mock()
    ...
    inventory_repo.get_transactions_by_sku.return_value = [
        InventoryTransaction("GIFT01", Decimal('10'), Decimal('50000'), date(2026, 4, 1), "IN")
    ]
    policy_service.get_current_policy.return_value = AccountingPolicyDTO(
        inventory_valuation_method=ValuationMethod.FIFO,
        effective_date="2026-01-01"
    ...
    dto = SalesInvoiceDTO(
        ...
        line_items=[
            ...
            SalesLineItem("GIFT01", "Quà tặng", Decimal('1'), Decimal('0'))  # Sai: unit_price = 0
        ]
    )
    ...
    # Kiểm tra công nợ
    total_debt = debt_repo.get_total_balance(f"CUST-{dto.buyer_tax_code}")
    expected_debt = Decimal('5000000') + (Decimal('5000000') * Decimal('0.1'))
    assert total_debt == expected_debt
...
def test_promotional_item_with_reasonable_market_value():
    ...
    Yêu cầu TT 99:
    - Doanh thu = Tổng giá trị hàng hóa thực tế
    - Công nợ = Tổng giá trị + Thuế GTGT
    """
    # 1. Chuẩn bị dependencies
    inventory_repo = Mock()
    journal_repo = Mock()
    ...
    inventory_repo.get_transactions_by_sku.return_value = [
        InventoryTransaction("PHONE01", Decimal('5'), Decimal('7000000'), date(2026, 4, 1), "IN"),
        InventoryTransaction("CASE01", Decimal('10'), Decimal('30000'), date(2026, 4, 1), "IN")
    ]
    policy_service.get_current_policy.return_value = AccountingPolicyDTO(
        inventory_valuation_method=ValuationMethod.FIFO,
        effective_date="2026-01-01"
    ...
    # 2. Tạo hóa đơn với hàng KM có giá trị hợp lý
    dto = SalesInvoiceDTO(
        ...
        line_items=[
            SalesLineItem("PHONE01", "Điện thoại", Decimal('1'), Decimal('10000000')),
            SalesLineItem("CASE01", "Ốp lưng (KM)", Decimal('1'), Decimal('80000'))  # Giá trị hợp lý
        ]
    )
    ...
    expected_debt = expected_total + (expected_total * Decimal('0.1'))
