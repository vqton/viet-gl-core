...
MOCK_TK_111 = TaiKhoan(so_tai_khoan="111", ten_tai_khoan="Tiền mặt", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN, la_tai_khoan_tong_hop=False)
MOCK_TK_331 = TaiKhoan(so_tai_khoan="331", ten_tai_khoan="Phải trả người bán", loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA, la_tai_khoan_tong_hop=False)
...
def setup_and_teardown_db():
    ...
    db.add(SQLAccount(so_tai_khoan="111", ten_tai_khoan="Tiền mặt", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN, cap_tai_khoan=1, la_tai_khoan_tong_hop=False))
    db.add(SQLAccount(so_tai_khoan="331", ten_tai_khoan="Phải trả người bán", loai_tai_khoan=LoaiTaiKhoan.NO_PHAI_TRA, cap_tai_khoan=1, la_tai_khoan_tong_hop=False))
    db.commit()
    db.close()
    yield
    teardown_test_db()
...
class TestDomainLogic:
    ...
    def test_journal_entry_khong_can_bang(self):
        ...
        lines = [
            JournalEntryLine(so_tai_khoan="111", no=Decimal(100), co=Decimal(0)),
            JournalEntryLine(so_tai_khoan="331", no=Decimal(0), co=Decimal(99)), # Thiếu 1
        ]
    ...
    def test_journal_line_no_va_co_cung_lon_hon_khong(self):
        ...
        lines = [
            JournalEntryLine(so_tai_khoan="111", no=Decimal(100), co=Decimal(1)) # Lỗi ở đây
        ]
        with pytest.raises(ValueError, match="chỉ được ghi Nợ hoặc Có, không đồng thời cả hai"):
    ...
    def test_ky_ke_toan_ngay_bat_dau_sau_ket_thuc(self):
        ...
        with pytest.raises(ValueError, match="Ngày bắt đầu không thể sau ngày kết thúc"):
    ...
    def test_ky_ke_toan_trang_thai_khong_hop_le(self):
        ...
        with pytest.raises(ValueError, match="Trạng thái kỳ phải là 'Open' hoặc 'Locked'"):
...
class TestApplicationServices:
    ...
    def test_tai_khoan_service_tao_tk_cap_con_thanh_cong(self, db_session):
        ...
        tk_con = TaiKhoan(so_tai_khoan="1112", ten_tai_khoan="Tiền mặt VND", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN, cap_tai_khoan=2, so_tai_khoan_cha="111", la_tai_khoan_tong_hop=False)
    ...
    def test_tai_khoan_service_tao_tk_cap_con_that_bai(self, db_session):
        ...
        tk_con = TaiKhoan(so_tai_khoan="1113", ten_tai_khoan="Tiền mặt USD", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN, cap_tai_khoan=2, so_tai_khoan_cha="999", la_tai_khoan_tong_hop=False)
        with pytest.raises(ValueError, match="Tài khoản cha '999' không tồn tại."):
    ...
    def test_accounting_period_service_khoa_ky_that_bai_vi_con_phieu_nhap(self, db_session):
        ...
        with pytest.raises(ValueError, match="vẫn còn 1 bút toán ở trạng thái 'Draft'"):
...
class TestInfrastructureRepositories:
    ...
    def test_account_repository_add_and_get(self, db_session):
        ...
        repo = AccountRepository(db_session)
        # 1. Tạo Domain Entity
        new_account = TaiKhoan(so_tai_khoan="131", ten_tai_khoan="Phải thu khách hàng", loai_tai_khoan=LoaiTaiKhoan.TAI_SAN)
        ...
        assert retrieved_account.la_tai_khoan_tong_hop is True # Giá trị mặc định
